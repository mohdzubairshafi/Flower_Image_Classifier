...
parser.add_argument('data_directory', action="store", nargs='?',
                    default="flowers", help="dataset directory")

# Set directory to save checkpoints
parser.add_argument('--save_dir', action="store", default="",
                    help="saving directory for checkpoint", dest="save_directory")
...
parser.add_argument('--epochs', action="store", default=3,
                    type=int, help="set epochs", dest="epochs")
...
print("Data directory: root/{}/ \nSave directory: root/{} \nArchitecture: {} ".format(
    arg_data_dir, arg_save_dir, arg_architecture))
...
if 102 <= arg_hidden_units[2] <= arg_hidden_units[1] <= arg_hidden_units[0] <= 25088:
    ...
    print("Hidden units are incompatible with the model. Default hidden units {} will be used".format(
        arg_hidden_units))
    print()
...
model.classifier = nn.Sequential(nn.Linear(25088, arg_hidden_units[0]),
                                 ...
                                 nn.Dropout(0.2),
                                 nn.Linear(
                                     arg_hidden_units[0], arg_hidden_units[1]),
                                 nn.ReLU(),
                                 nn.Dropout(0.2),
                                 nn.Linear(
                                     arg_hidden_units[1], arg_hidden_units[2]),
                                 nn.ReLU(),
                                 nn.Dropout(0.2),
...
if arg_save_dir:
    if not os.path.exists(arg_save_dir):
        ...
        print("Directory ", arg_save_dir,
              " has been created for saving checkpoints")
    else:
        print("Directory ", arg_save_dir,
              " allready exists for saving checkpoints")
