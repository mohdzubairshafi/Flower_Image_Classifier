...
def process_image(image):
    ...
    np_image = (
        np_image - np.array([0.485, 0.456, 0.406]))/np.array([0.229, 0.224, 0.225])
    # return transpose
...
# Get path of image
parser.add_argument('path_to_image', action="store", nargs='?',
                    default='flowers/test/1/image_06743.jpg', help="path/to/image")
# Get path of checkpoint
parser.add_argument('path_to_checkpoint', action="store", nargs='?',
                    default='checkpoint.pth', help="path/to/checkpoint")
# set top_k
parser.add_argument('--top_k', action="store", default=1,
                    type=int, help="enter number of guesses", dest="top_k")
# Choose json file:
parser.add_argument('--category_names', action="store",
                    default="cat_to_name.json", help="get json file", dest="category_names")
# Set GPU
parser.add_argument('--gpu', action="store_true",
                    default=False, help="Select GPU", dest="gpu")
...
print()
print("Path of image: {} \nPath of checkpoint: {} \nTopk: {} \nCategory names: {} ".format(
    arg_path_to_image, arg_path_to_checkpoint, arg_top_k, arg_category_names))
print('GPU: ', arg_gpu)
print()
...
my_model, my_optimizer, input_size, output_size, epoch = load_checkpoint(
    arg_path_to_checkpoint)
...
for count in range(arg_top_k):
    print('{} ...........{:.3f} %'.format(
        cat_to_name[idx_to_class[classes[0, count].item()]], probs[0, count].item()))
